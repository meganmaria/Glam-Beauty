...
public class MascaraActivity extends AppCompatActivity {
        ...
        ApiManager.getInstance().getMakeupService().getMakeUp().enqueue(new Callback<ArrayList<MakeUpList>>() {
            ...
            public void onResponse(Call<ArrayList<MakeUpList>> call, Response<ArrayList<MakeUpList>> response) {
                if (response.isSuccessful() && response.body() != null && !response.body().isEmpty()) {
                    Log.d("itemFromResponse", "onResponse mascara : " + response.body().get(8).getProduct_image_link());
                    for (int i = 0; i < response.body().size(); i++) {
                        if (response.body().get(i).getProduct_type().equals("mascara") && !response.body().get(i).getDescription().equals("")) {
                    ...
                    Log.d("itemFromResponse", "onResponse: mascara first->" + response.body().get(5).getName());
                }
    ...
    public void setRecyclerViewMakeUp(ArrayList<MakeUpList> makeUps) {
        RecyclerView recyclerView = findViewById(R.id.mascara_recycler_view);
